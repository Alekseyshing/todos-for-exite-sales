...
//             <Task projectId={projectId} key={uuidv4()} subtasks={subtask.subtasks} id={subtask.id} title={subtask.name} done={false} />
//           </div>
//         ))}
//         {subtasks && subtasks.map((subtask) => (
//           <div className={styles.subtask} key={subtask.id}>
...
const Task = ({ projectId, id, title, subtasks, done, parentId }) => {
  ...
  // const handleDelete = () => {
  //   if (parentId) {
  //     dispatch(deleteTask({ projectId, taskId: parentId, subtaskId: subtaskId }));
  //   } else {
  ...
  const renderSubtasks = (subtasks) => {
    if (subtasks.length > 0) {
      return subtasks.map((subtask) => (
        <div className={styles.subtask} key={subtask.id}>
          <Task projectId={projectId} subtasks={subtask.subtasks} id={subtask.id} title={subtask.name} done={subtask.done} />
        </div>
      ));
    }
  }
